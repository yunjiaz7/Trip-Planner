...
onMounted(async () => {
  console.log('🔍 [前端调试] Result页面加载')
  const data = sessionStorage.getItem('tripPlan')
  console.log('🔍 [前端调试] sessionStorage数据:', data ? `存在，长度: ${data.length}` : '不存在')
  
  if (data) {
    try {
      tripPlan.value = JSON.parse(data)
      console.log('🔍 [前端调试] 解析成功，tripPlan:', {
        city: tripPlan.value?.city,
        daysCount: tripPlan.value?.days?.length,
        hasWeather: !!tripPlan.value?.weather_info,
        hasBudget: !!tripPlan.value?.budget
      })
